...
const SharePage = () => {
  ...
  useEffect(() => {
    if (taskParam) {
      try {
        ...
        if (!isHexColor(task.color)) {
          setError(true);
          setErrorDetails("Formato de color de tarea no válido.");
          return;
        }
        if (task.category) {
          task.category.forEach((taskCategory) => {
            if (!isHexColor(taskCategory.color)) {
              setError(true);
              setErrorDetails("Formato de color de categoría no válido.");
              return;
            }
          });
        }
        ...
        console.error("Error dal decodificar datos de la tarea:", error);
        setErrorDetails("Error al descodificar los datos de la tarea." + error);
        setError(true);
      }
    }
    ...
    if (userNameParam) {
      ...
      if (decodedUserName.length > USER_NAME_MAX_LENGTH) {
        setError(true);
        setErrorDetails("Nombre de usuario demasiado largo.");
  ...
  useEffect(() => {
    document.title = `Task Pro - Tarea Recibida ${taskData ? "(" + taskData.name + ")" : ""}`;
  }, [taskData]);
