...
const ImportExport = () => {
  ...
  useEffect(() => {
    document.title = "Task Pro - Transferir Tareas";
  }, []);
  ...
  const handleExportAll = () => {
    ...
    showToast(`Se exporto todas las tareas (${user.tasks.length})`);
  };
  ...
  const handleImportFromLink = async (): Promise<void> => {
    try {
      ...
      if (text.startsWith(`${location.protocol}//${location.hostname}`)) {
        ...
        showToast(
          <div>
            No se ha podido importar la tarea desde el enlace proporcionado. Asegúrese de que el enlace se ha copiado correctamente.
          </div>,
          { type: "error" }
        );
      }
    ...
    <>
      ...
      <ManagementContainer>
        {user.tasks.length > 0 ? (
          ...
          <h3 style={{ opacity: 0.8, fontStyle: "italic" }}>No tienes treas que exportar.</h3>
        )}
      </ManagementContainer>
      ...
      <ManagementButtonsContainer>
        <Tooltip
          title={
            selectedTasks.length > 0
              ? `Tareas Seleccionadas: ${new Intl.ListFormat("en", {
                  style: "long",
                  type: "conjunction",
                }).format(
        ...
        >
          <ManagementButton onClick={handleExport} disabled={selectedTasks.length === 0}>
            <FileDownload /> &nbsp; Exportar tarea seleccionada a JSON{" "}
        ...
        <h2 style={{ textAlign: "center" }}>Importar Tareas de JSON</h2>
        ...
        {/Windows|Linux|Macintosh/i.test(navigator.userAgent) && (
          <div style={{ width: "300px" }}>
            ...
            >
              ...
              <div>Suelte aquí el archivo JSON para importar tareas</div>
            </DropZone>
          </div>
        )}
