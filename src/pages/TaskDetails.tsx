...
const TaskDetails = () => {
  ...
  useEffect(() => {
    document.title = `Task Pro - ${task?.name || "Detalles de la Tarea"}`;
  }, [task?.name]);
      ...
      <Container>
        ...
        <TaskTable>
          <tbody>
            ...
            {task?.deadline && (
              <TableRow>
                <TableHeader>Plazo de la tarea:</TableHeader>
                <TableData>{dateFormatter.format(new Date(task.deadline))}</TableData>
              </TableRow>
            )}
            <TableRow>
              <TableHeader>Listo:</TableHeader>
              <TableData>
            ...
            {task.category && task.category.length > 0 && (
              <TableRow>
                ...
                <TableData>
                  ...
                  <CategoryContainer>
  {task && task.category && task.category.map((category) => (
    <CategoryBadge key={category.id} category={category} glow={false} />
  ))}
</CategoryContainer>}
                </TableData>
